...
class pulse_seq():
	...
	if len(np.shape(c))==2: n_pulses /= 2
	...
	tx_i = self.amplify(filtered_sequence, 0.95)
	...
	if len(np.shape(c))==1: sig = np.concatenate((np.abs(c),np.ones([5])))
	if len(np.shape(c))==2: 
		sig = np.concatenate((np.abs(c[0,:]),np.ones([5])))
		n_pulses *= 2
	on = self.buffer(sig, self.sym_rate, self.tr_buff)
	...
	#plt.plot(self.tx_i[20000:25000]); plt.plot(30000*self.tr[20000:25000] ); plt.show()
    ...
    def append(self, c, sym_rate, buff):
    	return np.concatenate((c, (np.zeros([int(buff * sym_rate)]))))
	...
	n_ones = int(buff * sym_rate)
    	#ones = [1.] * n_ones
    	return np.concatenate((c, np.ones(n_ones)))
	...
	if len(c.shape)==1:
		pulselen = np.size(c)
	if len(c.shape)==2:
		pulselen = np.size(c)/2
	t_pulse = pulselen / float(sym_rate)
	t_period = 1 / float(pulse_rep_freq)
	t_rx = t_period - t_pulse
	print "n_period:", t_period*sym_rate
	zeros = np.zeros([np.round(t_rx * sym_rate)])
	if len(c.shape)==1:
		return np.concatenate((c, zeros))
	if len(c.shape)==2:
		a = np.concatenate((c[0,:],zeros))
		b = np.concatenate((c[1,:],zeros))
		print "n_period:", np.size(a)+np.size(b)
	...
	return np.convolve(in_signal, filter_taps[::-1], mode='same')
...
def get_options():
    ...
    parser.add_option("-f", "--prf", type="float", default=200,
    ...
    parser.add_option("-b", "--buff-time", type="float", default=140,
...
def write_to_file():
    ...
    print np.size(p_code['CCS_8'],0)
    print np.size(p_code['CCS_8'],1)
    plt.plot(10000*pulse.tr[0:4000:])
    plt.plot(pulse.tx_i[0:4000]); plt.show()
    #plt.plot(pulse.tx_i); plt.show()
